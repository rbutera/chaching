...
<div class="prototype" class:ledger={variant === 'B'} class:lanes={variant === 'C'}>
	...
	<main>
		{#if scene === 'loading'}
			<section class="cold" aria-live="polite"><span class="eyebrow">Opening the register</span><h1>Counting your sins…</h1><p>Cold-scanning Claude Code transcripts.</p><div class="scan-line"></div><p class="muted">First load streams every session file once.<br/>This is the only slow part.</p><button onclick={() => setParam('state','overview')}>Preview loaded dashboard →</button></section>
		{:else if section === 'Settings'}
			<div class="page-title"><div><span class="eyebrow">The back office</span><h1>Settings</h1></div><span>Preview only</span></div>
		...
		{:else if section === 'Explore usage'}
			<div class="page-title"><div><span class="eyebrow">Follow the money</span><h1>Explore usage</h1></div><button onclick={() => section = 'Dashboard'}>Back to dashboard</button></div>
		...
		{:else}
			<div class="page-title"><div><span class="eyebrow">Your next limit, at a glance</span><h1>{variant === 'A' ? 'Room to keep going.' : variant === 'B' ? 'The account register.' : 'Two providers. One view.'}</h1></div><span class="fresh">Updated just now</span></div>
			...
			<div class="lower">
				...
				<aside class="receipt"><div class="section-heading"><span class="eyebrow">Subscription receipt</span><label>Period <select bind:value={period}><option>30 days</option></select></label></div><h2>Money well spent.</h2><div class="multiplier">9.7<span>×</span></div><p>API value across 3 Accounts</p><dl><div><dt>Usage value</dt><dd>$5,820.00</dd></div><div><dt>Account fees</dt><dd>$600.00</dd></div><div class="net"><dt>Difference</dt><dd>+$5,220.00</dd></div></dl><div class="average"><span>Average $/hour</span><b>$8.17</b></div><small>12 Aug–10 Sep 2026<br/>30 days ending today · 712 elapsed hours</small></aside>
			</div>
		{/if}
	</main>
	<footer>chaching <span>Keep the receipts.</span></footer>
</div>
