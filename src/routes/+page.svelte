<script lang="ts">
	// Throwaway: three spend-first layouts on /?variant=A|B|C, with fictional data.
	...
	<main>
		...
		{:else}
			...
			<div class="charts">
				...
				<section class="provider-chart"><div class="section-heading"><h2>By provider</h2><span>{windowDays} {windowDays===1 ? 'day' : 'days'}</span></div>{#each providerTotals as entry}<div class="provider-total"><div><span>{entry.name}</span><strong><MoneyOdometer amount={entry.value} size="sm" tone="default" reducedMotion={motion ? undefined : true}/></strong></div><meter min="0" max={Math.max(1,usage.periodCost)} value={entry.value} aria-label={`${entry.name} spend: ${money(entry.value)}`}></meter></div>{/each}</section>
			</div>
...
<style>
	...
	.current-focus{display:grid;grid-template-columns:1.2fr 1fr;border:1px solid var(--border)}.current-focus article{padding:24px 28px}.current-focus article+article{border-left:1px solid var(--border)}.current-focus article.attention{background:var(--accent-soft);border-top:3px solid var(--accent);padding-top:21px}.account-top{display:flex;align-items:start;justify-content:space-between;gap:16px}.account-name{display:flex;flex-direction:column;gap:3px}.account-name strong{font-size:var(--text-lg)}.account-name span,.available{font-size:var(--text-xs);color:var(--text-muted)}.active{font:var(--type-label);color:var(--accent);padding-top:5px}.quota-pair{display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-top:30px}.window-label{display:flex;justify-content:space-between;align-items:baseline;gap:10px;font-size:var(--text-sm)}.remaining{font-size:var(--text-xs);color:var(--text-muted)}.remaining b{font:700 30px/1.2 var(--font-sans);color:var(--text)}.low .remaining b{color:var(--accent)}meter{display:block;width:100%;height:7px;appearance:none;border:0;margin:12px 0;background:var(--border);border-radius:0}meter::-webkit-meter-bar{background:var(--border);border:0;border-radius:0;height:7px}meter::-webkit-meter-optimum-value{background:var(--paper-200)}.low meter::-webkit-meter-optimum-value{background:var(--accent)}meter::-moz-meter-bar{background:var(--paper-200)}small{font-size:var(--text-xs);color:var(--text-muted)}
