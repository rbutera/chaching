...
describe('runWizard (mocked prompts — TTY bypassed via isTTY stub)', () => {
	...
	vi.mocked(clack.select).mockResolvedValue('local');
...
describe('applySyncSetupToConfig', () => {
	it('keeps local SQLite as the explicit default without generating ids', () => {
		const result = applySyncSetupToConfig(defaultConfig(), { mode: 'local' });
		expect(result.sync).toEqual(defaultConfig().sync);
	});

	it('applies ids generated by an explicit create or join flow', () => {
		const result = applySyncSetupToConfig(defaultConfig(), {
			mode: 'sync',
