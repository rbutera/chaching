// Singleton ingestion service. ONE cold scan per server process (not per request),
// then fs.watch (recursive) + mtime-poll fallback to tail new lines. Maintains the
// Rollup and fans deltas out to SSE subscribers.
...
import type { CursorProviderConfig } from './config';
...
import type { RollupDelta, RollupSnapshot, UsageRecord } from '$lib/types';
...
class IngestionService {
	...
	private async start(): Promise<void> {
		...
		if (cfg.providers.cursor.enabled && cfg.providers.cursor.adminApiToken) {
			...
			this.startCursorPolling(cfg.providers.cursor);
		}
	...
	get stats() {
		return { coldScanMs: this.coldScanMs, files: this.fileStates.size, providerErrors: this.providerStatus.snapshot() };
	}
}

// True module-level singleton (one per Node process / SvelteKit server).
let instance: IngestionService | null = null;

export function getService(): IngestionService {
	if (!instance) instance = new IngestionService();
	return instance;
}
