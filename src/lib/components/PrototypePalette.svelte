...
<style>
	.palette-controls{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);z-index:var(--z-toast);display:flex;gap:8px;align-items:center;max-width:calc(100vw - 24px);padding:8px;background:var(--surface-3);color:var(--text);border:1px solid var(--border);border-radius:8px;box-shadow:var(--shadow)}
	label{display:grid;gap:2px;min-width:0}label>span{font-size:10px;color:var(--text-muted)}select{background:var(--surface-3);color:var(--text);max-width:100%;min-height:36px;border:0;font:inherit;font-size:13px}button{color:var(--text);cursor:pointer;min-width:36px;min-height:44px}button:focus-visible,select:focus-visible,summary:focus-visible{outline:2px solid var(--focus-ring);outline-offset:2px}
