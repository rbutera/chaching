...
<details class="mapping">
	...
	{#each Object.entries(semanticGroups) as [group, names]}
		<h3>{group}</h3><div class="swatches">{#each names as name}<div><span class="swatch" class:warning={name === 'warn'} style:background={vars[name]}></span><span>{name}<code>{vars[name]}</code></span></div>{/each}</div>
	{/each}
</details>
<style>
	...
	.swatch.warning{outline:1px solid var(--warn-ink);outline-offset:-1px}
</style>
