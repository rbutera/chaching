...
export function accountWindowValues({ grain, accounts, mappings, from, to, providers, machines, models, localAccounts = [] }: {
	...
	const rows: AccountValueRow[] = [...new Map(accounts.map(account => [account.id, account])).values()]
		.filter(account => (!providers.size || providers.has(account.provider)) &&
			(!machines.size || machineAccounts.has(account.id)))
		.map(account => ({ id: account.id, provider: account.provider, name: account.name, account: account.account,
			feeUsd: account.monthlyUsd === null || localAccounts.some(local => local.provider === account.provider && local.pendingLegacyIds?.length) ? null : account.monthlyUsd * days / FEE_PRORATA_DAYS }));
	const billedProviders = new Set(rows.map(row => row.provider));
	let valueUsd = 0;
	for (const usage of grain) {
		if (usage.day < from || usage.day > to || !billedProviders.has(usage.provider) ||
