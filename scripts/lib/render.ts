...
// real font bytes passed as Buffers (no system-font fallback). It is intended to
// run only from `scripts/*` (Node) — `@resvg/resvg-js` is a native binding and
// is marked external to Vite so it never reaches the browser/server bundle.
...
export async function svgFromElement(node: RenderNode, opts: RenderOptions): Promise<string> {
	return satori(node as unknown as React.ReactNode, {
		width: opts.width,
		height: opts.height,
