...
const packageRoot = process.env.CHACHING_PACKAGE_ROOT!;
const baseUrl = process.env.CHACHING_TEST_DATABASE_URL;
const enabled = baseUrl && process.env.CHACHING_TEST_PG_TOOLS === '1';
const suite = enabled ? describe : describe.skip;
suite.each([2, 3, 4, 5])('Packaged pool rollout from schema %i', sourceVersion => {
 ...
 let db: Pool;
 let url: string;
 const script = join(packageRoot, 'docs/pool-rollout.mjs');
 ...
 beforeAll(async () => {
  ...
  await db.query(readFileSync(new URL('../../core/src/sync/fixtures/pool-v3.sql', import.meta.url), 'utf8'));
  ...
  if (sourceVersion >= 4) {
   ...
   await db.query('DROP TABLE chaching_sync.machine_session_activity');
   if (sourceVersion === 4) await db.query('ALTER TABLE chaching_sync.machine_day_agg DROP COLUMN monetary');
   await db.query('UPDATE chaching_sync.schema_version SET version=$1', [sourceVersion]);
  }
 }, 30000);
 afterAll(async () => {
 ...
 it('backs up, rejects drift, migrates twice, verifies and restores schema/config', async () => {
  const packageName = execFileSync('npm', ['pack','--ignore-scripts','--silent','--pack-destination',directory], { encoding:'utf8', cwd: packageRoot }).trim();
  ...
  expect(() => run('preflight',rollout,rosterFile,artifact,join(packageRoot, 'bin/chaching.js'))).toThrow();
  writeFileSync(rosterFile, validRoster);
  expect(run('preflight',rollout,rosterFile,artifact,join(packageRoot, 'bin/chaching.js'))).toContain('verified');
