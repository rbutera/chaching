...
import { performSyncAction } from '@chaching/core/sync/manager';
...
import { defaultConfig, updateConfig, configFilePath, clearConfigCache } from '@chaching/core/config';
...
describe('provider command logic', () => {
	...
	it('provider disable cursor: cursor flipped off, other config intact', async () => {
		const { runProvider } = await import('./commands/provider');
		const logs: string[] = [];
		...
		clearConfigCache();
		const { loadConfig } = await import('@chaching/core/config');
