...
import {
	...
	performSyncAction,
	setSyncInterval
} from '@chaching/core/sync/manager';
	...
	if (command === 'schema') {
		...
		try {
			...
			if (rest.includes('--migrate')) {
				if (before !== 2 && before !== 3 && before !== 4 && before !== 5 && before !== SCHEMA_VERSION)
					throw new Error(`Schema migration requires version 2, 3, 4, 5 or ${SCHEMA_VERSION}; found ${before ?? 'missing'}.`);
				await store.open();
			}
