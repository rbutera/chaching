...
import { runOnce } from '@chaching/core/engine';
import { getReportAccountContext } from '@chaching/core/sync/manager';
import { getPricingMeta } from '@chaching/core/pricing/cost';
import { poolGrain } from '@chaching/shared/view-model';
import { sumGrain, filterDays } from '@chaching/shared/aggregate';
import type { Period } from '@chaching/shared/types';
import { noArt as resolveNoArt, receiptFooter } from '../theme/personality';
import { buildReceipt, rollingPeriodRange } from '@chaching/receipt/receipt/build';
...
import { renderReceiptText } from '@chaching/receipt/receipt/render-text';
import type { ReceiptJson } from '@chaching/receipt/receipt/model';
...
export async function runReceipt(flags: ReceiptFlags): Promise<void> {
	...
	const redacted = redactReceipt(model, { ...redactionContext(), redact: flags.redact });
	...
	if (flags.png) {
		...
		if (process.stdout.isTTY) {
			writeStdoutSync(renderReceiptText(redacted, { noArt, env:process.env }));
		} else {
 ...
 */
async function writePng(model: import('@chaching/receipt/receipt/model').ReceiptModel, flags: ReceiptFlags, period: Period): Promise<void> {
	const periodTag = period;
	const outPath = flags.pngPath ?? `./chaching-receipt-${periodTag}.png`;
	lastPngPath = outPath;

	let renderReceiptPng: typeof import('@chaching/receipt/receipt/render-png')['renderReceiptPng'];
	try {
		({ renderReceiptPng } = await import('@chaching/receipt/receipt/render-png'));
	} catch (err) {
		failPng(err);
	}

	try {
		const png = await renderReceiptPng(model, { assetRoot: assetRoot() });
