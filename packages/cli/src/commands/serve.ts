...
import { distributionRoot } from '@chaching/core/runtime-assets';
import { noArt, accent, dim } from '../theme/personality';
import { bannerLine } from '../tui/theme';
import { normalizeBasePath } from '@chaching/shared/base-path';
...
export async function runServe(): Promise<void> {
	const rootDir = distributionRoot();
	const buildEntry = join(rootDir, 'server', 'index.js');

	if (!existsSync(buildEntry)) {
		console.error('chaching: build artifact missing. Run `pnpm build` before `chaching serve`.');
		process.exit(1);
	}
