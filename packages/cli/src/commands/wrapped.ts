...
import { buildWrapped } from '@chaching/receipt/wrapped/build';
import { redactWrapped } from '@chaching/receipt/wrapped/redact';
import {currentAccount,redactionContext} from '@chaching/receipt/receipt/node-context';
import { renderWrappedText } from '@chaching/receipt/wrapped/render-text';
import type { WrappedJson } from '@chaching/receipt/wrapped/model';
...
export async function runWrapped(flags: WrappedFlags): Promise<void> {
	...
	const redacted = redactWrapped(model, { ...redactionContext(), redact: flags.redact });
	...
	if (flags.png) {
		...
		if (process.stdout.isTTY) {
			writeStdoutSync(renderWrappedText(redacted, { noArt, env:process.env }));
		} else {
 ...
 */
async function writePng(model: import('@chaching/receipt/wrapped/model').WrappedModel, flags: WrappedFlags): Promise<void> {
	const outPath = flags.pngPath ?? `./chaching-wrapped-${model.month}.png`;
	lastPngPath = outPath;

	let renderWrappedPng: typeof import('@chaching/receipt/wrapped/render-png')['renderWrappedPng'];
	try {
		({ renderWrappedPng } = await import('@chaching/receipt/wrapped/render-png'));
	} catch (err) {
		failPng(err);
	}

	try {
		const png = await renderWrappedPng(model, { assetRoot: assetRoot() });
