...
import { accountConfigProblems, accountFeesByProvider } from '@chaching/shared/accounts';
...
describe('config', () => {
	...
	it('gives concurrent processes the same migrated Account IDs', async () => {
		...
		try {
			...
			await build({ entry: ['src/config.ts'], noExternal: [/^@chaching\//], outDir: join(dir, 'bundle'), format: ['esm'], platform: 'node', silent: true, dts: false, outExtension: () => ({ js: '.mjs' }) });
