...
import type { DayModelAgg, RollupSnapshot, TokenCounts } from '@chaching/shared/types';
import { filterDays, sumGrain } from '@chaching/shared/aggregate';
import { buildReceipt, rollingPeriodRange } from './build';
import { periodWindow } from '@chaching/shared/view-model';
...
describe('buildReceipt — sections + invariants', () => {
	...
	it('coupons use retained monetary savings and never change total burn', () => {
		const retained = grain.map(row => row.model === 'claude-opus-4-8' ? {...row,monetary:{input:0,output:0,cacheCreation:0,cacheRead:2,cacheReadUncached:10,tools:0}} : row);
		const m = buildReceipt(snapFrom(retained), {now:FIXED_NOW});
		expect(m.youSaved).toBe(8);
		expect(m.coupons).toHaveLength(1);
		expect(m.coupons[0]).toMatchObject({actualCost:2,wouldHaveCost:10,saved:8});
		expect(m.totalBurn).toBeCloseTo(sumGrain(grain).cost,10);
		expect(buildReceipt(snap,{now:FIXED_NOW}).coupons).toEqual([]);
	});
...
it('filters model usage and coupons while retaining the whole selected Account fee', () => {
	...
	for (const row of snapshot.dayModel) row.monetary = {input:row.cost-1,output:0,cacheCreation:0,cacheRead:1,cacheReadUncached:5,tools:0};
	const options = {
		now: FIXED_NOW,
