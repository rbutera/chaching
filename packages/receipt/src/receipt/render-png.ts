...
// woffs in static/fonts/ (400 + 700 + 800), loaded from the explicit asset root supplied to this
...
import { money, compactTokens, providerLabel, int } from '@chaching/shared/format';
import { renderPng, svgToPngDataUri, type RenderNode } from './png-pipeline';
import { barWidths, GOLD_MARK_SVG, RANGE_ARROW_SVG, PAPER } from './assets';
 ...
 * saw-tooth strips.
 */
export async function renderReceiptPng(model: ReceiptModel, {assetRoot}: {assetRoot: string}): Promise<Buffer> {
	const regular = readFileSync(join(assetRoot, 'fonts', 'jetbrains-mono-latin-400-normal.woff'));
	const bold = readFileSync(join(assetRoot, 'fonts', 'jetbrains-mono-latin-700-normal.woff'));
	const heavy = readFileSync(join(assetRoot, 'fonts', 'jetbrains-mono-latin-800-normal.woff'));
