...
- Local-only Account filters now use the canonical local roster and the same single-mapping attribution as pooled views. Snapshot and live deltas retain Account IDs; selected Account fees follow the same scope. Browser checks cover mapped spend, an unused Account with an unknown fee and persisted selection. Offline retention of pooled Account selections still needs completion; local IDs can differ from deduplicated pool IDs.
