...
This procedure upgrades schema 2, 3, 4 or 5 to 6 using the same migration as Chaching. It affects **every pool in the database**. Do not start upgraded clients before backup, and do not start old clients against schema 6. No live rollout has been performed for PR 31.
...
Migration calls `chaching sync schema --migrate --clients-stopped`. It does not start sync or publish usage. PostgreSQL applies the existing DDL in one transaction. Migration is repeatable with the same rollout directory. Both commands verify schema 6 and unchanged normalized row inventories. Changed fees, links, usage or machine membership fail verification instead of being accepted as the new baseline.
