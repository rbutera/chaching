...
vi.mock('@chaching/receipt/receipt/redact', async (orig) => {
	const real = (await orig()) as typeof import('@chaching/receipt/receipt/redact');
	return {
		...real,
...
vi.mock('@chaching/receipt/receipt/render-png', () => ({
...
import { renderReceiptPng } from '@chaching/receipt/receipt/render-png';
...
describe('/api/receipt.png', () => {
	...
	it('redaction is OPT-IN: default call does NOT redact', async () => {
		await call('');
		expect(redactCalls.at(-1)).toMatchObject({ redact: false });
	});

	it('?redact=1 turns redaction ON', async () => {
		await call('?redact=1');
		expect(redactCalls.at(-1)).toMatchObject({ redact: true });
	});
