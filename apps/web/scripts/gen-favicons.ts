...
const brandAssetsDir = dirname(fileURLToPath(import.meta.resolve('@chaching/shared/brand/assets/logo.svg')));
...
async function main() {
	...
	// Ship the horizontal lockup into static/ so the README hero (and any other
	// consumer) references an asset that is actually in the npm tarball — `src/`
	// is excluded from the package `files`, so a repo-relative logo path 404s on
	// npmjs.com.
	await copyFile(join(brandAssetsDir, 'logo.svg'), join(staticDir, 'logo.svg'));

	console.log('[gen-favicons] wrote favicon.ico, apple-touch-icon.png, icon-192/512.png, manifest.webmanifest, logo.svg');
}
