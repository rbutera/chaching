...
test('rejects a green run with missing or skipped mandatory integration tests', () => {
  ...
  const testResults = [result('serve.test.ts'), result('engine.sync.integration.test.ts'), result('sync/store.integration.test.ts'), result('sync/migration.integration.test.ts'), result('pool-rollout.integration.test.ts', 3)];
